...
export function FilaAprovacaoList({ itens }: { itens: ItemFilaAprovacao[] }) {
  const [itemEmDecisao, setItemEmDecisao] = useState<ItemFilaAprovacao | null>(null);
  ...
  return (
    <>
            ...
            <tr key={item.id}>
              ...
              <td>
                {item.statusAprovacao === 'pendente' && (
                  <button type="button" onClick={() => setItemEmDecisao(item)}>
                    Decidir
                  </button>
                )}
              </td>
      ...
      {itemEmDecisao && <GateAprovacaoModal item={itemEmDecisao} onFechar={() => setItemEmDecisao(null)} />}
    </>
  );
}
