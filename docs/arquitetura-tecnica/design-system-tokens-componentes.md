...
| 11 | **Aviso de Gap (API)** | `Nível=Crítico\|Atenção` | Adicionado durante RAZ-102 (telas core): nenhum componente do v1 cobria "sinalizar lacuna de API na tela" — busca em `search_design_system` (10 componentes-núcleo + biblioteca "Simple Design System" já anexada ao arquivo) não retornou nada equivalente, então foi criado como componente novo em vez de recriado ad-hoc por tela. `Crítico` (vermelho, `state/danger-*`) é o banner de topo de tela quando o endpoint/schema não existe; `Atenção` (âmbar, `state/warning-*`) é para nota inline de menor severidade. Página própria `10 Aviso de Gap (API)`, entre `09 Resumo Fail-soft` e as telas do RAZ-102 |
| 12 | **Tabela — Balancete** | (sem variantes — colunas fixas) | Adicionado durante RAZ-112 (consultas/relatórios): busca em `search_design_system` mostrou que "Table" na biblioteca "Simple Design System" já anexada é só um **ícone** (40×40), não um componente de dados — nada cobria uma tabela de relatório somente-leitura. Diferente do componente 7 (`Tabela — Seleção em Lote`, específico do lote de pagamento com checkbox), este espelha `BalanceteResponse`/`LinhaBalancete` (RAZ-101) campo a campo — Código/Descrição/Saldo anterior/Mov. Débito/Mov. Crédito/Saldo atual, mais rodapé com totais e um indicador "Confere" (Σdébito=Σcrédito, `state/success-*`/`state/danger-*`) que materializa `Balancete.confere()`. Mesma paleta/tipografia do componente 7 (variáveis de cor reaproveitadas diretamente) para consistência visual entre as duas tabelas do sistema. Página própria `11 Tabela — Balancete (RAZ-112)` |

Cobertura (antes de RAZ-112): 91 variáveis (43 primitivas + 35 semânticas + 13 espaçamento/raio), 9 text styles, 3 effect styles, 11 component sets / 27 variantes. RAZ-112 acrescenta 1 componente (`Tabela — Balancete`, sem variantes) — inventário completo não recontado nesta revisão.
