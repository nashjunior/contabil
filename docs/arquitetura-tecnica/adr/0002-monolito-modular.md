...
- **Decisão:** **Monólito modular** — núcleo transacional (razão, execução, fechamento) num único deployable com uma transação ACID por fato; borda de publicação/integração assíncrona. Módulos com **fronteiras internas explícitas** (execução, razão, plataforma) para permitir extração futura. **Convenção de camadas:** interfaces de **repository/persistência no `domain`** (o domínio declara seus contratos); use cases (`application`) são **POJOs sem anotação de framework**; a **`infra` faz o wiring (`@Configuration`/`@Bean`) e detém a transação** (borda) — o `application` fica livre de Spring/JPA.
