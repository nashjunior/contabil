...
| [0053](./0053-rbac-dotacao-liquidacao-papeis-fixantes.md) | Matriz RBAC concede `execucao:dotacao` (`CRIAR`/`ALTERAR`) a `ADMIN_PLATAFORMA` e `execucao:liquidacao` (`CRIAR`) a `LANCADOR` — fecha o fluxo do operador ponta a ponta sem misturar "fixa o teto" com "gasta contra o teto" | Aceita |
| [0054](./0054-mecanica-postagem-ddr-e-fonte-recurso-no-lancamento.md) | Mecânica da DDR (classes 7/8) e da fonte de recursos (FR): `FR` é coluna `fonte_recurso` (nullable, condicional por conta) no `lancamento` + VO `FonteRecurso`; partidas de controle DDR entram no **mesmo** `FatoContabil` do evento (reusa Σ=Σ, sem 2º motor); `DisponibilidadePorFonte` é read model derivado (RAZ-225, resolve o *como* deixado aberto por ADR-0044/0050) | Aceita |
| [0055](./0055-ux-gate-aprovacao-decisao-modal-mapeamento-erro.md) | UX da decisão do gate 4-eyes (aprovar/devolver): modal disparado por linha pendente na fila (não tela de detalhe), resumo+clique como confirmação (sem segundo modal), motivo condicional via zod, e mapeamento corrigido `auto_aprovacao_vedada`=409/`sem_permissao`=403 (RAZ-236) | Aceita |
| [0056](./0056-msc-core-modulo-prestacao-contas-read-model.md) | MSC core (RAZ-251) mora num módulo `prestacao-contas` dedicado (fora de `razao-*`, isola Apache POI/XBRL/leiaute SICONFI do ledger); consome o razão como read model no padrão `BalancetePort` (ADR-0007/0047, port `razao-domain` estendido pela dimensão IC, `Acao.LER`); a linha da MSC é DTO **long-format** = unpivot do tripleto de `LinhaBalancete`; validador local (5 regras SICONFI) sobre o DTO. Projeção SQL + IC-condicional-por-conta dependem de RAZ-250/ADR-0050 | Aceita |
| [0057](./0057-modelagem-persistencia-ic-restantes-msc.md) | Modelagem/persistência das IC restantes da MSC (RAZ-250, resolve o *como* deixado por ADR-0050): cada IC capturável é **coluna própria tipada nullable** — `CO/NR/ND/FS/AI` no `lancamento` (VO agregado `InformacoesComplementares` que absorve a `FonteRecurso`), `PO` no `fato_contabil`; **FP/DC são derivadas** na projeção de saldos DDR (cl.7/8), sem coluna; regra "conta exige IC" na escrituração; `AI` vem de RP (RAZ-207); enum `InformacaoComplementar` (9 IC × origem). Destrava a projeção de ADR-0056 | Aceita |
| [0059](./0059-transparencia-ativa-read-model-publico-portal-api-dados-abertos.md) | Transparência ativa (F1): consumidor real do outbox = **projetor** para read model público **append-only** (substitui `BrokerEntregaIndisponivel`); portal + API de dados abertos JSON/CSV + bulk CDN + dicionário sobre ele; fronteira público×interno **física** (role SELECT-only + RLS + payload mascarado); borda rate-limit/CDN; **guarda de mascaramento que quebra o build** em todos os canais (D5). Incorpora refinamentos vinculantes do gate RAZ-272. Impl = RAZ-273 (blockedBy). Ratificação Proposta→Aceita gated nos 3 accepts + revalidação Dec. 10.540 arts. 7º/8º | Proposta |
| [0058](./0058-conformidade-validar-iti-evidencia-operacional.md) | Conformidade VALIDAR/ITI (RAZ-249, resíduo 2/2 de RAZ-208) é **evidência operacional, não integração no adapter** (o `validar.iti.gov.br` só valida por upload/QR Code e emite relatório humano — sem API M2M estável, cf. ADR-0049): ratifica a fronteira de `verificar(...)` (checagem local integridade+cadeia+revogação) e anexa o relatório VALIDAR ao **dossiê do documento** no object store amarrado ao `idTransacao`/hash já gravados na trilha; owner da 1ª evidência é ENTE-PILOTO/OWNER; evento `conformidade_validar_iti` na hash-chain é gancho futuro (F2 = DOC-ICP-15 automatizado) | Aceita |
| [0060](./0060-portal-publico-transparencia-shell-piso-acessibilidade-downloads.md) | Portal público de transparência (RAZ-278, wireframe sobre o contrato real de RAZ-273/ADR-0059): shell/rota **separados** do back-office (sem sessão, path-scoped por ente, sem `Seletor de Ente`); piso de acessibilidade = **WCAG 2.2 AA/eMAG** reaproveitando os tokens semânticos SIAFIC já validados em AA (biblioteca gov.br/ds indisponível no workspace Figma, verificado via `get_libraries` — revisão futura se/quando existir); Lista e Totais são a **mesma tela** com barra de filtros compartilhada (UX-2); duas affordances de download **distintas e rotuladas** (CSV da consulta filtrada × base completa via CDN não filtrada, D4); gap real do nome de exibição do ente (contrato só expõe `enteId`) sinalizado, não fabricado | Aceita |

> **Nota:** o número **0019 não foi utilizado** — reservado durante execução paralela de agentes e nunca materializado em arquivo (o **mesmo** padrão-raiz endereçado pelo [ADR-0037](./0037-isolamento-e-guarda-commits-concorrentes-agentes.md)). Não renumerar os ADRs existentes (a numeração é histórica/imutável, mesmo princípio do ADR); o próximo ADR novo continua a partir do 0061.
