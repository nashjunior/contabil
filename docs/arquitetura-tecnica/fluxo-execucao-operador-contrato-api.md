...
└───────────────────────────────────────────────────────────────────┘
   ...
   [gov.br — autenticação + assinatura fora do SIAFIC]
        │ 302 → /assinatura/oauth/callback (backend, PKCE) → 302 → rota fixa do SPA (RAZ-158; ver §6.10)
        ▼
...
→ ratificado (RAZ-152): 302 para uma rota FIXA do SPA (ex.: FRONTEND_RETORNO_URI/execucao/assinatura/retorno?resultado=ok|erro[&codigo=…],
  configurável, nunca hardcoded), que aí sim chama o backend/mostra o estado "Retornando do gov.br" (§5.5).
  O redirect_uri REGISTRADO no gov.br (AssinaturaGovBrOAuthProperties.redirectUri) NÃO muda — continua = este callback
  (PKCE exige a troca code→token server-side); muda só a resposta do callback. A rota é fixa (sem {id}): o fluxo OAuth é
  empenho-agnóstico (FluxoPendente só tem state/codeVerifier/ente), então o SPA restaura o empenho-alvo do sessionStorage.
```
`redirect_uri` (`AssinaturaGovBrOAuthProperties.redirectUri`) não tem default — mas **permanece = o backend `/assinatura/oauth/callback`** (R1 da ratificação): o PKCE exige a troca `code`→`token` server-side, então o navegador volta ao backend primeiro; o que muda é só a **resposta** (204/JSON → 302 para a rota fixa do SPA `frontend-retorno-uri`). Sem essa rota-alvo que pertença ao SPA, não existe onde renderizar "Retornando do gov.br" nem como voltar para a Tela 1b — o operador literalmente para numa aba com JSON ou em branco depois de autenticar no gov.br.
...
- **Assinatura do empenho (§5.5/§6.10, RAZ-141): escrita pronta em `master`, leitura tem 3 gaps de backend — ratificados e delegados.** `AssinarEmpenho`/`AssinaturaEmpenhoController`/OAuth (RAZ-103, ADR-0017/0027) funcionam; faltam (1) `GET .../empenhos/{id}/documento` — bridge HTTP para `ArmazenamentoDocumentos.ler(URI)`, sem o qual não há pré-visualização real de PDF; (2) enriquecer a leitura do empenho (`GET /empenhos/{id}` não existe; a listagem RAZ-121 não devolve `documentoPendenteUri`/hash/idTransacao/nível); (3) o callback OAuth de assinatura devolve hoje 204/JSON cru ao navegador do gov.br — sem uma rota fixa do SPA como alvo do `302`, não há onde renderizar "Retornando do gov.br" nem como voltar à tela. **Ratificado em [ADR-0039](./adr/0039-contrato-leitura-assinatura-empenho-preview-retorno-oauth.md) (Aceita, RAZ-152, 4 refinamentos vinculantes) e delegado a [RAZ-156] (`GET /empenhos/{id}`) / [RAZ-157] (`.../documento`) / [RAZ-158] (callback `302` → rota fixa do SPA).** Figma (páginas `17`/`18`, RAZ-141) já desenha os 5 estados (pendente/confirmação/retorno/assinado/rejeitada) contra este contrato, com "Aviso de Gap" inline nas duas telas que dependem dos endpoints ausentes — mesmo padrão de RAZ-102/110/112/140.
