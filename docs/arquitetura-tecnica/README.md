...
| **Aplicação (monólito modular)** | Núcleo + serviços de plataforma | Um deployable transacional; módulos com fronteiras internas — [execução da despesa (domínio)](./execucao-orcamentaria-despesa.md) · [fluxo do operador + contrato de API](./fluxo-execucao-operador-contrato-api.md) · [motor de razão (domínio)](./motor-razao-partidas-dobradas.md) |
