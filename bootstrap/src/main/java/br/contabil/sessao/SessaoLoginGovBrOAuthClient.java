...
final class SessaoLoginGovBrOAuthClient implements ClienteTokenSessaoLoginGovBr {
    ...
    public SessaoLoginGovBrOAuthToken trocarCodigoPorToken(String code, String codeVerifier) {
        ...
        try {
            ...
            throw new IllegalStateException("Falha de rede ao trocar code OIDC por token gov.br de login", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Interrompido ao trocar code OIDC por token gov.br de login", e);
        }

        if (resposta.statusCode() < 200 || resposta.statusCode() >= 300) {
            throw new IllegalStateException("Token endpoint gov.br de login respondeu status " + resposta.statusCode());
        }
        return extrairToken(resposta.body());
    }

    private SessaoLoginGovBrOAuthToken extrairToken(String corpo) {
        try {
            JsonNode json = objectMapper.readTree(corpo);
            JsonNode accessToken = json.get("access_token");
            if (accessToken == null || accessToken.asText().isBlank()) {
                throw new IllegalStateException("Token endpoint gov.br de login nao retornou access_token");
            }
            long expiresIn = json.path("expires_in").asLong(EXPIRACAO_PADRAO.toSeconds());
            return new SessaoLoginGovBrOAuthToken(accessToken.asText(), clock.instant().plusSeconds(expiresIn));
        } catch (IOException e) {
            throw new IllegalStateException("Token endpoint gov.br de login retornou JSON invalido", e);
        }
    }
